    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('code').addEventListener('keydown', function(event) {
            if (event.key === 'Enter') {
                ...
                // Clear the fields
                document.getElementById('student-name')?.classList?.remove("hidden");
                document.getElementById('student-picture')?.classList?.remove("hidden");
                ...
                document.getElementById("my-4")?.classList?.add("my-4");
                document.getElementById("my-4")?.classList?.add("my-8");
                document.getElementById("goldBadge")?.classList?.add("hidden");
                document.getElementById("goldMadelDiv")?.classList?.add("hidden");
                document.getElementById("goldMadelLine")?.classList?.add("hidden");
                const urlParams = new URLSearchParams(document.getElementById('code').value.toLowerCase().split('?')[1]);
                this.value = '';
                console.log(urlParams.get('code'))
                updateStudentCard(urlParams.get('code'));
            }
        });
    });
}
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === "success") {
                    ...
                    profileCard.classList.remove("fade-in-left-profile-card");
                    void profileCard.offsetWidth;
                    profileCard.classList.add("fade-in-left-profile-card");
                    document.getElementById("passing-year").classList.remove("hidden");


                    let congrats = document.getElementById("congrats")
                    congrats.classList.remove("fade-in-top-animation");
                    void congrats.offsetWidth;
                    congrats.classList.add("fade-in-top-animation");

                    let studentPicture = document.getElementById("student-picture")
                    studentPicture.classList.remove("fade-in-animation");
                    void studentPicture.offsetWidth;
                    studentPicture.classList.add("fade-in-animation");

                    let profileDetails = document.getElementById("profile-details")
                    profileDetails.classList.remove("fade-in-animation");
                    void profileDetails.offsetWidth;
                    profileDetails.classList.add("fade-in-animation");
                } else {
...
function updateStudentCard(studentCode) {
    getStudentDetails(studentCode)
        .then(student => {
            ...
            //Please change the base url for 7th convocation with the ending slash
            let pictureBaseUrl = "https://8th-convocation-pictures.s3.ap-southeast-1.amazonaws.com/";


            if (student) {
                document.getElementById('student-name').innerText = student.name;
                document.getElementById('student-picture').src = pictureBaseUrl + student.picture;
                document.getElementById('passing-year-2').innerText = student.completion_semester;
